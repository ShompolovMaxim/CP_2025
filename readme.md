...
* поэлементное расстояние вычисляется по следующим принципам
  ...
  * если столбец имеет тип числовых значений
    ...
    * если один элемент - диапазон (пусть второй), то ((a-b1)^2+(a-b2)^2) / (M-m)^2
    * если оба - диапазоны, то ((a1-b1)^2+(a2-b2)^2) / (M-m)^2
  * если столбец имеет тип порядковых значений
    ...
    * если один элемент - диапазон (пусть второй), то ((r(a)-r(b1))^2+(r(a)-r(b2))^2) / (n-1)^2
    * если оба - диапазоны, то ((r(a1)-r(b1))^2+(r(a2)-r(b2))^2) / (n-1)^2
...
* затем берётся первая группа, не удовлетворяющая метрике
...
* quasi_identifiers_types - тип каждого столбца (массив размера равного количеству квазиидентификаторов со значениями 'real', 'ordered', 'unordered'); по умолчанию значения считаются номинальными
* scale - величина задающая разброс значений при генерации методом по умолчанию (на неё умножается сдвиг по индексу для порядковых значений и численный сдвиг для вещественных значений)
#### Результат работы алгоритма
...
* генерация методом по умолчанию производится по следующим принципам:
  ...
  * для порядковых значений генерируется сдвиг и берётся значение из упорядоченного массива (с повторениями), индекс которого равен сумме индекса изначального значения и сдвига
  * для численных значений строится гистограмма, из неё выбирается (с повторениями) количество значений, равное количеству строк, а затем генерация происходит аналогично генерации для порядковых значений, но вместо индекса изначального элемента берётся индекс элемента, ближайшего к нему
