        ...
        if column_type == 'unordered':
            ...
            while i < len(values):
                ...
                if k_lines < group_size:
                    group_values[-1] = group_values[-1] + values[pred_i:]
                else:
                    group_values.append(values[pred_i:i])
                pred_i = i
            for group in group_values:
                generalized_value = GeneralizationRange(None, None, 'unordered', group) if len(group) != 1 else group[0]
                for value in group:
